...
select urun_adi,max(urun_miktari)as maximum_urun_mik,min(urun_miktari) as minimum_urun_mik from manav 
where urun_adi is not null group by urun_adi order by maximum_urun_mik;
...
select isim , sum(urun_miktari) as toplam_urun from  manav group by isim,urun_adi order by isim desc;
...
select cinsiyet,round(avg(maas)) as ortalama_maas from personel group by cinsiyet; 
...
select ulke,cinsiyet, count(*) as calisan_sayisi,round(avg(yas)) as yas_ortalamasi from personel where maas>3000 group by ulke,cinsiyet;

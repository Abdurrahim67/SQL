...
select ulke, round(avg(maas),2) as maas_ortalamasi from personel where cinsiyet='E'
group by ulke
...
select ulke, round(avg(maas),2) as ortalama_maas from personel where cinsiyet='E' group by ulke having count(cinsiyet)>1;
...
//1.yol
select maas as max_ikinci_maas from isciler order by maas desc limit 1 offset 1;
//2.yol
select max(maas) as max_ikinci_maas from isciler where maas<(select max(maas) from isciler); 
...
//2.yol
select*from isciler where maas=(select max(maas) as max_ikinci_maas from isciler where maas<(select max(maas) from isciler)); 
